...
public:
    ...
    void endTest() {
        ...
        double logs_per_second = static_cast<double>(total_logs_.load()) * 1000.0 / duration;
        std::cout << "Logs per second: " << std::fixed << std::setprecision(2) << logs_per_second << "\n";
        std::cout << "---------------------------------------------\n";
    }
...
// 工作线程函数
void worker_thread(int thread_id, int iterations, ThreadBarrier& barrier, bool use_random_delay) {
    // 准备随机延迟生成器
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        // 记录不同级别的日志
        MM_DEBUG("[Thread-%03d] Iteration %d/%d: Processing data from sensor %d with size %d", 
        ...
        if (i % 50 == 0) {
            MM_WARN("[Thread-%03d] Warning: Sensor %d reading is unstable at iteration %d", 
                    thread_id, sensor_id, i + 1);
            LoggerPerfStats::Instance().incrementLogCount();
        }

        if (i % 200 == 0) {
            MM_ERROR("[Thread-%03d] Error: Failed to process data from sensor %d at iteration %d", 
                    thread_id, sensor_id, i + 1);
            LoggerPerfStats::Instance().incrementLogCount();
        }
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < num_threads; ++i) {
        threads.emplace_back(worker_thread, i, iterations, std::ref(barrier), use_random_delay);
    }
