...
void PerformanceTest::init_mm_logger() {
  mm_log::Logger::Instance().Initialize(config_.log_prefix,
      config_.max_file_size_mb, config_.max_total_size_mb, config_.enable_debug,
      config_.enable_console, config_.enable_file, config_.queue_size,
...
void PerformanceTest::init_spdlog() {
  ...
  if (config_.enable_console) {
    ...
    console_sink->set_level(
        config_.enable_debug ? spdlog::level::debug : spdlog::level::info);
  ...
  auto logger = std::make_shared<spdlog::async_logger>("spdlog_logger",
...
void PerformanceTest::generate_test_messages() {
  try {
    ...
    switch (config_.message_size) {
      case MessageSize::SMALL: target_size = 64; break;
      case MessageSize::MEDIUM: target_size = 256; break;
      case MessageSize::LARGE: target_size = 1024; break;
    }

    // Base message template
    std::string base_message =
        "Performance test message from thread %d, iteration %d, with payload: ";

    if (config_.verbose) {
      std::cout << "Generating test message with target size: " << target_size
                << " bytes" << std::endl;
    }

    // Calculate payload size to reach target_size
    size_t payload_size = target_size - base_message.size() -
                          20;  // 20 for numbers and formatting

    if (payload_size > 10000) {
      // 限制有效载荷大小，防止过大造成问题
      payload_size = 10000;
      if (config_.verbose) {
        std::cout << "Limited payload size to " << payload_size << " bytes"
                  << std::endl;
      }
    }

    // Generate random payload
    std::string payload = utils::generate_random_string(payload_size);
    test_message_       = base_message;
    test_message_.append(payload);
  } catch (const std::exception& e) {
    std::cerr << "Error generating test messages: " << e.what() << std::endl;
    // 设置一个默认消息，防止程序崩溃
    test_message_ = "Performance test message from thread %d, iteration %d";
  }
}
...
void PerformanceTest::log_message(int thread_id, int iteration, bool flush) {
  if (config_.use_mm_logger) {
    switch (thread_id % 4) {
      case 0: MM_DEBUG(test_message_.c_str(), thread_id, iteration); break;
      case 1: MM_INFO(test_message_.c_str(), thread_id, iteration); break;
      case 2: MM_WARN(test_message_.c_str(), thread_id, iteration); break;
      case 3: MM_ERROR(test_message_.c_str(), thread_id, iteration); break;
    }
  } else {
    switch (thread_id % 4) {
      case 0: spdlog::debug(test_message_.c_str(), thread_id, iteration); break;
      case 1: spdlog::info(test_message_.c_str(), thread_id, iteration); break;
      case 2: spdlog::warn(test_message_.c_str(), thread_id, iteration); break;
      case 3: spdlog::error(test_message_.c_str(), thread_id, iteration); break;
    }
  }

  // Flush is a no-op for now since we don't have a direct flush API in
  // mm_logger and spdlog::flush() is deprecated
  (void)flush;
}
...
PerfResult PerformanceTest::run_throughput_test() {
  ...
  if (config_.verbose) {
    std::cout << "Starting throughput test with " << config_.num_threads
              << " threads, " << config_.iterations << " iterations per thread"
              << std::endl;
  }
  ...
  result.logs_per_second =
      (completed_logs.load() * 1000.0) / result.total_time_ms;
...
PerfResult PerformanceTest::run_latency_test() {
  ...
  if (config_.verbose) {
    std::cout << "Starting latency test with " << config_.num_threads
              << " threads, " << config_.iterations << " iterations per thread"
              << std::endl;
  }
  ...
  for (int i = 0; i < config_.num_threads; ++i) {
    threads.emplace_back([this, i, &barrier, &latencies, &latencies_mutex]() {
      ...
      // Merge results
      {
        ...
        latencies.insert(
            latencies.end(), thread_latencies.begin(), thread_latencies.end());
      }
    });
  }
...
PerfResult PerformanceTest::run_stress_test(int burst_size, int burst_count) {
  ...
  if (config_.verbose) {
    std::cout << "Starting stress test with " << config_.num_threads
              << " threads, " << burst_count << " bursts of " << burst_size
              << " logs each" << std::endl;
  }
  ...
  for (int i = 0; i < config_.num_threads; ++i) {
    threads.emplace_back(
        [this, i, burst_size, burst_count, &barrier, &completed_logs]() {
          // Wait for all threads to be ready
          barrier.wait();
...
void PerformanceTest::write_csv_results(
    const PerfResult& result, const std::string& test_type) {
  ...
  if (!file_exists) {
    // Write header
    csv_file << "Timestamp,TestName,TestType,Logger,Threads,Iterations,"
                "MessageSize,QueueSize,WorkerThreads,EnableConsole,EnableFile,"
             << "TotalTime_ms,LogsPerSecond,Min_Latency_us,Median_Latency_us,"
                "P95_Latency_us,P99_Latency_us,Max_Latency_us,Memory_KB"
             << std::endl;
  }
  ...
  // Write data row
  csv_file << timestamp << "," << config_.test_name << "," << test_type << ","
           << (config_.use_mm_logger ? "mm_logger" : "spdlog") << ","
           << config_.num_threads << "," << config_.iterations << ","
           << get_message_size_name(config_.message_size) << ","
           << config_.queue_size << "," << config_.worker_threads << ","
           << (config_.enable_console ? "true" : "false") << ","
           << (config_.enable_file ? "true" : "false") << "," << std::fixed
           << std::setprecision(2) << result.total_time_ms << "," << std::fixed
           << std::setprecision(2) << result.logs_per_second << ","
           ...
           << "," << result.memory_used_kb << std::endl;
